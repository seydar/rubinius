...
class Hash
  ...
  class Iterator
    ...
    def next(entry)
      @order[entry ? entry.order + 1 : 0]
    end
  end

  def to_iter
    Iterator.new @entries, __order__, @capacity
  end
  ...
  def each_entry
    return __order__.to_enum :each unless block_given?

    __order__.each { |x| yield x }
  end
end
